<?php
    ...
    if (chkLogin($dbConnection))
    {
        ...
        if(isset($_POST["token"], $_POST["device"])) {
            ...
            if($stmt = $dbConnection->prepare("SELECT token, deviceID FROM computers WHERE id = ? AND token = ? LIMIT 1"))
            {
                ...
                if($stmt->num_rows == 1) {
                    ...
                    if ($inStmt = $dbConnection->prepare("INSERT INTO computers (name, token, id) VALUES (?, ?, ?)")) {
